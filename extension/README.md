...
Opening a SyncWatch room automatically configures its room code and role. When the host sets or changes a link, the extension opens or updates one stream window for every participant. The host's strongest HTML5 video candidate becomes the controller automatically; viewer windows follow it. To show the local stream inside SyncWatch, click the extension on that stream tab and choose **Show this video in SyncWatch**. Chrome requires this one user invocation before tab capture. A controller command is shown as applied only after the real player acknowledges its state.
