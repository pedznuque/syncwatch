...
The extension popup includes **Open synchronized stream window** and **Use this tab as controller**. It reads the room's current stream link, opens it in a separate browser window, detects the primary HTML5 video, and ensures only the selected tab/frame publishes host playback state.
