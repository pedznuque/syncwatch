...
The extension popup includes **Open synchronized stream window**, **Use this tab as controller**, and an automatic full-window video option. It reads the room's current stream link, opens it in a separate browser window, detects the strongest HTML5 video candidate, and ensures only the selected tab/frame publishes host playback state. A controller command is shown as applied only after that player acknowledges its real state.
