...
The running SyncWatch app provides a **Download extension** button, plus **Open stream window** and **Render in SyncWatch** controls after a web link is set. The extension popup can also open the room's current stream in a dedicated synchronized window.

SyncWatch shows whether no link is set, a link is waiting for the extension, or an HTML5 video was detected. Some providers block iframe embedding; for those sites, keep the dedicated stream window open and use SyncWatch as the remote control.
